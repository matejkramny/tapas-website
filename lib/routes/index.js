...
function charge (distance) {
	if (distance <= 3) return 1.5; else return Math.ceil(distance)*0.5
}
	...
	calculateDistance(req.body.customer.postcode, function (result) {
		...
		async.each(req.body.items, function (reqitem, cb) {
			...
			for (var i = 0; i < items.length; i++) {
				...
				text += '\n--------------\n ' + items[i].order + " - " + items[i].name + " - "+ req.body.items[i].quantity + "x (" + (items[i].price*req.body.items[i].quantity).toFixed(2) + " GBP)";
