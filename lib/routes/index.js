...
function getDistance(req, res) {
	...
	var request = {
		origin: 'OX2 9DU',
		destination: req.params.postcode
	};
	gmAPI.directions(request, function (err, result){
		if (result.status == "OK") res.send(String((result.routes[0].legs[0].distance.value/1609.34).toFixed(2))).end(); else res.status(400).end()
	});
}
...
function submitOrder (req, res) {
	...
	async.each(req.body.items, function (reqitem, cb) {
		...
		for (var i = 0; i < items.length; i++) {
			text += '\n--------------\n ' + items[i].order + " - " + items[i].name + " - "+ req.body.items[i].quantity + "x";
