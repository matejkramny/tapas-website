...
function charge (distance) {
	if (distance <= 2) return 1; else return Math.ceil(distance)*0.5
}
	...
	calculateDistance(req.body.customer.postcode, function (result) {
		...
		async.each(req.body.items, function (reqitem, cb) {
			...
			for (var i = 0; i < items.length; i++) {
				...
				var dbIngredients = [];
				if (req.body.items[i].ingredients instanceof Array) {
					for (var x = 0; x < req.body.items[i].ingredients.length; x++) {
						dbIngredients.push({
							id: req.body.items[i].id,
							name: items[i].ingredients[x].name,
							default_quantity: items[i].ingredients[x].default_quantity,
							value: req.body.items[i].ingredients[x].value
						});
					}
				}
